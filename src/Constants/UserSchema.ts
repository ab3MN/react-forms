...
const userSchema = yup.object().shape({
  ...
  image: yup
    ...
    .test(
      'is-valid-format',
      'The image format must be jpg, png, jpeg or webp',
      (file): boolean => file instanceof FileList && validImageExtensions.includes(file[0].type),
    )
    .test(
      'is-valid-size',
      'Max size allowed is 1KB',
      (file) => file instanceof FileList && file[0].size <= MAX_FILE_SIZE,
    ),
