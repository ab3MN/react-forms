    ...
    <section>
      ...
      <form onSubmit={onSubmitHandler}>
        <div className='input__container'>
          <label htmlFor='name'>Name</label> <input id='name' type='text' autoComplete='on' name='name' />
          <p className='form__error'>{errors.name}</p>
        </div>
        <div className='input__container'>
          <label htmlFor='age'>Age</label> <input id='age' type='number' autoComplete='on' name='number' />
          <p className='form__error'>{errors.age}</p>
        </div>
        <div className='input__container'>
          <label htmlFor='email'>Email</label> <input id='email' type='email' autoComplete='on' name='email' />
          <p className='form__error'>{errors.email}</p>
        </div>
        <div className='input__container'>
          <label htmlFor='password'>Password</label>
          <input id='password' type='password' autoComplete='on' name='password' onChange={onChangePassword} />
        ...
        <div className='input__container'>
          ...
          <input id='repeatPassword' type='password' autoComplete='on' name='repeatPassword' />{' '}
          <p className='form__error'>{errors.repeatPassword}</p>
        </div>
        <h3>Gender</h3>
        <div>
          <label htmlFor='female'>Female</label> <input id='female' type='radio' value='female' name='gender' />
          <label htmlFor='male'>Male</label>
          <input id='male' type='radio' value='male' name='gender' /> <p className='form__error'>{errors.gender}</p>
        </div>
        <div className='input__container'>
          <label htmlFor='conditions'>
            I accept Terms and Conditions agreement
            <input id='conditions' type='checkbox' name='conditions' />{' '}
            <p className='form__error'>{errors.conditions}</p>
          </label>
        </div>
        <div className='input__container'>
          <label htmlFor='image'>Upload an image</label>
          <input type='file' accept='.jpeg,.jpg,.png,.webp' id='image' name='image' />{' '}
          <p className='form__error'>{errors.image}</p>
        </div>
        <div className='input__container'>
          <label htmlFor='country'>Country:</label>{' '}
          <input type='text' id='country' list='countriesList' name='country' autoComplete='on' />
          <datalist id='countriesList'>
            {countriesList.map((country) => (
              <option key={country} value={country} />
            ))}
          </datalist>{' '}
          <p className='form__error'>{errors.country}</p>
        </div>
